...
class KiwiEmitter {
  ...
  constructor(options) {
    ...
    if (options?.includeRejections && typeof options?.includeRejections !== "boolean") new _EmitterError.default(`'${options.includeRejections}' is not Boolean.`, {
      name: "TypeError"
    }).throw();
    if (options?.listenerLimit && typeof options?.listenerLimit !== "number") new _EmitterError.default(`'${options.listenerLimit}' is not Number.`, {
      name: "TypeError"
    }).throw();
  ...
  #options = {
    listenerCount: _nodeEvents.default.defaultMaxListeners,
  ...
  on(name, fncallback) {
    ...
    if (typeof fncallback !== "function") new _EmitterError.default(`'${fncallback}' is not Function.`, {
      name: "TypeError"
    }).throw();
  ...
  emitted(name) {
    ...
    const events = this.events.get(name);
    for (const event of events) {
      if (event?.emitted) __emitted = true;else break;
    }
    ;
    return __emitted;
  }
  ...
  suspendListener(name, time) {
    ...
    if (typeof time !== "numvwe") new _EmitterError.default(`'${time}' is not Number.`, {
      name: "TypeError"
    }).throw();
    if (!this.has(name)) new _EmitterError.default(`'${name}' is not available.`, {
      name: "InvalidEvent"
    }).throw();
